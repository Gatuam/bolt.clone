...
const ChatView = () => {
  ...
  return (
    <div className="bg-[#00000023] min-h-[98vh] border border-[#1dd9ff22] py-3 px-2 rounded-lg flex flex-col justify-between relative overflow-hidden">
      {}
      <div className="overflow-y-auto px-1 flex-1 flex flex-col gap-4 w-full [&::-webkit-scrollbar]:hidden">
        {message.map((msg, i) => (
          <div
            key={i}
            className={`${
              msg.role === "ai" && "self-end w-full max-w-[80%]"
            } max-w-[70%] bg-[#202020f5] text-neutral-300 p-3 rounded-sm border border-[#27bbf614] text-sm`}
          >
